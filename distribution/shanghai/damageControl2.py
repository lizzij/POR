...
## Problem: probably duplicate URL
users = get_page_as_df('allUsers', page_columns_class['allUsers'])
newUsers = users.loc[(users.user_id >= 41197000) & (users.user_id <= 41208000) & (users.day == 0)]
newUsers['URL'] = URL + "shanghai/" + newUsers.user_id_hashid + "/" + newUsers.day_hashid + "/info"
df = newUsers[['user_id', 'wechat_id', 'URL']]
df.to_csv('newUsers.csv', encoding='utf-8-sig', index=False)
